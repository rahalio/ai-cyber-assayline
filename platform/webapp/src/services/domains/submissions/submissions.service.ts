...
const rawSubmissionsService = {
   ...
   * List Submissions
   */
  async getSubmission(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/submissions` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Create Submission
   */
  async createSubmission(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/submissions`;
   ...
   */
  async getCompletenessCheck(submissionId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/submissions/${submissionId}/completeness-check`;
   ...
   * Return Submission
   */
  async getReturn(submissionId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/submissions/${submissionId}/return`;
