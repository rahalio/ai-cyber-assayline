 ...
 *
 * API client for conservatism domain.
...
const rawConservatismService = {
  /**
   * List Conservatism Overlays
   */
  async getConservatismOverlay(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/conservatism-overlays` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getConservatismOverlay(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/conservatism-overlays`;
   ...
   */
  async getAggregate(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/conservatism-overlays/aggregate` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const conservatismService = makeService(rawConservatismService, "conservatism");
