...
const rawTieringService = {
  /**
   * List Tier Assessments
   */
  async getTierAssessment(modelId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/models/${modelId}/tier-assessments` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Assess Model Tier
   */
  async getTierAssessment(modelId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/models/${modelId}/tier-assessments`;
   ...
   * Get Tier Criteria
   */
  async getTierCriterium(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/tier-criteria` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async updateTierCriterium(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/tier-criteria`;
...
export const tieringService = makeService(rawTieringService, "tiering");
