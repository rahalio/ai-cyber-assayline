...
const rawMonitoringService = {
   ...
   */
  async getObligation(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/monitoring/obligations` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getObligation(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/monitoring/obligations`;
   ...
   */
  async getBreach(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }
   ...
   */
  async getRevalidationTrigger(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/revalidation-triggers` + (params ? `?${new URLSearchParams(params).toString()}` : '');
